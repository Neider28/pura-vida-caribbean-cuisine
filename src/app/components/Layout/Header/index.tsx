...
const Header: React.FC = () => {
  ...
  return (
    <header
      ...
      <div
        ref={mobileMenuRef}
        className={`fixed top-0 right-0 z-50 h-full w-full max-w-xs transform bg-white shadow-lg transition-transform duration-300 lg:hidden ${
        ...
        <div className='flex items-center justify-between gap-2 p-4'>
          ...
          <button
            ...
            <Icon
              ...
              width={24}
              height={24}
              className='me-2 inline-block text-24 text-black hover:text-primary'
            />
          </button>
        </div>
        ...
        <nav className='flex flex-col items-start p-4'>
          ...
          <div className='mt-4 w-full'>
            <Link
              href='#menu'
              onClick={() => setNavbarOpen(false)}
              className='block rounded-lg border border-primary bg-primary px-4 py-3 text-center font-semibold text-white transition duration-300 ease-in-out hover:bg-transparent hover:text-primary'>
              {content.ui.orderOnline}
            </Link>
          </div>
        </nav>
